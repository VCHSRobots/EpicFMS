...
class SliderUnit():
    ...
    def destroy(self):
        # Call this to destroy/kill this object. The background loop is stopped. Then you can 
    ...
    def is_door_open(self):
        # Use current status to determine if the door is open.
        # If the unit is off-line, or comm problem, False is
        # reported.
        self.updatelock.acquire()
        valid = self.validstatus
        if valid:
            s = self.status["slider"]
        else:
            s = "Unknown"
        self.updatelock.release()
    ...
    def close_door(self):
        ...
        log("Slider Unit %d commanded to close door." % self.sliderid)
    ...
    def open_door(self):
        ...
        # successfully put in the command queue.  Use status to 
        # see that it acutally occured sometime later.
        log("Slider Unit %d commanded to open door." % self.sliderid)
    ...
    def center_door(self):
        ...
        # successfully put in the command queue.  Use status to 
        # see that it acutally occured sometime later.
        log("Slider Unit %d commanded to center door." % self.sliderid)
    ...
    def set_game_mode(self, gmode):
        ...
        log("Slider Unit %d set to game mode %s." % (self.sliderid, gmode))
    ...
    def blink(self):
        ...
        log("Slider Unit %d commanded to blink." % self.sliderid)
    ...
    def run_detector_test(self):
        ...
        log("Slider Unit %d commanded to run self test." % self.sliderid)
    ...
    def reset_hits_on_unit(self):
        ...
        log("Slider Unit %d commanded to reset hits on hardware." % self.sliderid)
    ...
    def reset_hits(self):
        ...
        self.hits = 0
        self.updatelock.release()
        log("Slider Unit %d commanded to reset hits in software." % self.sliderid)
    ...
    def get_raw_hits(self):
        # Returns the number of hits recorded in the lastest status
        # report, or zero if error or not connected.
        s = self.get_rawstatus()
